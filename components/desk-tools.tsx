...
export function DeskTools({onTeam,onJob}:{onTeam:(members:TeamMember[])=>void;onJob:(id:string)=>void}){
 ...
 return <section className="desk-tools">{error&&<p className="error" role="alert">{error}</p>}{message&&<p className="notice" role="status">{message}</p>}{(me?.role==='supervisor'||me?.role==='admin')&&<GuildQualitySummary/>}<div className="supervisor-alerts"><h2>Job alerts {alerts.filter(n=>!n.read_at).length>0&&<span>({alerts.filter(n=>!n.read_at).length} unread)</span>}</h2>{alerts.length===0?<p>No unread job alerts from the last 24 hours.</p>:alerts.map(n=><div key={n.id} className={'supervisor-alert'+(!n.read_at?' unread':'')}><button onClick={()=>{onJob(n.job_id);action(n.id,'read')}}><strong>{n.message}</strong><span>{n.recipient_name||'Field supervisor'} · {new Date(n.created).toLocaleString()}</span><small>{n.push_status==='sent'?'Push accepted by notification service':n.push_status==='not_enabled'?'Recipient has not enabled phone notifications':n.push_status==='failed'?'Phone push failed — in-app alert saved':'Push pending'}</small></button><Button variant="outline" onClick={()=>action(n.id,'read')}>Read</Button>{['failed','pending','not_enabled'].includes(n.push_status)&&<Button variant="outline" onClick={()=>action(n.id,'retry')}>Retry push</Button>}</div>)}</div></section>
}
