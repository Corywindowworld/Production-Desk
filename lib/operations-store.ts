...
export async function operation(m:Member,input:any){
 ...
 await db.transaction(async tx=>{
  ...
  if(action==='create'){assert(j.product==='Diamond Screens'||!j.screenCount,'Screen quantities require a Diamond Screens account.',400);assert(!(j.product==='Windows'&&j.entryDoorCount>0)&&!(j.product!=='Windows'&&(j.windowCount>0||j.slidingDoors>0))&&!(j.product==='Diamond Screens'&&j.entryDoorCount>0),'Windows/SPD, Entry Doors, and Diamond Screens require separate accounts.',400);await checkSupervisor();history='Customer created manually';}
   ...
   edit();const s=parse(scheduleSchema,input.data);assert(!s.adminOverride||reviewer(m),'Field Supervisor or Administrator access required for a schedule override.');const schedulingCrew=await tx.prepare("SELECT supervisor_id FROM members WHERE id=? AND role='installer' AND active=1").bind(s.installerId).first();assert(schedulingCrew?.supervisor_id,'Assign a Field Supervisor in the installer profile first.',400);const schedulingSupervisor=await tx.prepare("SELECT id FROM members WHERE id=? AND role IN ('admin','supervisor') AND active=1").bind(schedulingCrew.supervisor_id).first();assert(schedulingSupervisor,'Installer needs an active Field Supervisor.',400);assert(!s.endDate||s.endDate===s.date||remainingWorkdays(s.date,s.endDate)>0,'Select a multi-day range that includes a weekday.',400);assert(j.permitReceived&&j.permitNumber,'Permit Received and a permit number are required before scheduling.',400);
   ...
   if((dayDifference(j.received,s.date)??0)>30&&s.adminOverride){assert(j.stage==='Received'||(m.role==='admin'&&j.amount===0),'Unpaid scheduling overrides are available for RCVD jobs.',400);await confirmSchedule(s);history=`${m.role==='admin'?'Admin':'Field Supervisor'} approved schedule beyond 30 days. Payment requirement overridden; balance due: ${j.amount??'unknown'}. Appointment: ${s.date}`}else if((dayDifference(j.received,s.date)??0)>30){assert(j.amount===0&&s.paymentReference,'Beyond 30 days requires a zero amount due and payment reference.',400);o.pendingSchedule={...s,requestedBy:m.name,requestedAt:at};history='Schedule requested beyond 30 days; awaiting FS/Admin approval';await alert(schedulingCrew.supervisor_id,`Job #${j.number}: schedule beyond 30 days needs approval.`)}
   else{await confirmSchedule(s);history=`Scheduled ${s.date}${s.endDate&&s.endDate!==s.date?' through '+s.endDate:''} ${s.time?hourLabel(s.time):s.period}, stop ${s.stop}`}
   ...
   if(action==='approveSchedule'){assert(j.amount===0&&o.pendingSchedule.paymentReference,'Payment in full must be recorded before approval.',400);await confirmSchedule(o.pendingSchedule);history='Schedule beyond 30 days approved'}else{o.pendingSchedule=null;history='Schedule request declined'}
