...
export async function operation(m:Member,input:any){
 ...
 if(action==='agingDates'){
  ...
  await db.transaction(async tx=>{
   for(const r of [...batch.rows].sort((a,b)=>a.id.localeCompare(b.id))){
    ...
    assert(r.key===expected&&job.paymentMethod!=='PO','Only the current aging date of an eligible job may be edited here.',400);
 ...
 await db.transaction(async tx=>{
  ...
  if(action==='create'){assert(j.product==='Diamond Screens'||!j.screenCount,'Screen quantities require a Diamond Screens account.',400);assert(!(j.product==='Windows'&&j.entryDoorCount>0)&&!(j.product!=='Windows'&&(j.windowCount>0||j.slidingDoors>0))&&!(j.product==='Diamond Screens'&&j.entryDoorCount>0),'Windows/SPD, Entry Doors, and Diamond Screens require separate accounts.',400);await checkSupervisor();history='Customer created manually';}
   ...
   edit();const s=parse(scheduleSchema,input.data);assert(s.date>=today,'Choose today or a future date.',400);const schedulingCrew=await tx.prepare("SELECT supervisor_id FROM members WHERE id=? AND role='installer' AND active=1").bind(s.installerId).first();assert(schedulingCrew?.supervisor_id,'Assign a Field Supervisor in the installer profile first.',400);const schedulingSupervisor=await tx.prepare("SELECT id FROM members WHERE id=? AND role IN ('admin','supervisor') AND active=1").bind(schedulingCrew.supervisor_id).first();assert(schedulingSupervisor,'Installer needs an active Field Supervisor.',400);assert(!s.endDate||s.endDate===s.date||remainingWorkdays(s.date,s.endDate)>0,'Select a multi-day range that includes a weekday.',400);assert(j.permitReceived&&j.permitNumber,'Permit Received and a permit number are required before scheduling.',400);
