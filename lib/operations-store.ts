...
export async function operation(m:Member,input:any){
 ...
 await db.transaction(async tx=>{
  ...
  if(action==='create'){assert(j.product==='Diamond Screens'||!j.screenCount,'Screen quantities require a Diamond Screens account.',400);assert(!(j.product==='Windows'&&j.entryDoorCount>0)&&!(j.product!=='Windows'&&(j.windowCount>0||j.slidingDoors>0))&&!(j.product==='Diamond Screens'&&j.entryDoorCount>0),'Windows/SPD, Entry Doors, and Diamond Screens require separate accounts.',400);await checkSupervisor();history='Customer created manually';}
   ...
   assert(m.role==='admin','Only an Administrator may record payments.');const paymentType=parse(z.enum(['FNC','CHK','CC','AQUA','PO']),input.data.paymentType);const amount=parse(z.number().finite().min(0).multipleOf(.01),input.data.amount),reference=parse(z.string().trim().min(1).max(300),input.data.reference);
   assert(j.amount!=null&&amount<=j.amount,'A payment must not increase the balance. Ask an administrator to reconcile unknown balances.',400);history=`Amount due ${j.amount} → ${amount}. Payment type: ${paymentType}. Payment reference: ${reference}`;j.amount=amount;j.paymentMethod=paymentType;
   ...
   else{await confirmSchedule(s);history=`Scheduled ${s.date}${s.endDate&&s.endDate!==s.date?' through '+s.endDate:''} ${s.period}, stop ${s.stop}`}
