...
export function aging(j:any,today=localDay()){
 const stage=normalizedJobStatus(j),kind=stage==='Received'?'Received':['Production','InProgress'].includes(stage)?'Production':stage==='Incomplete'?'Incomplete':null;
...
// Only normalize legacy empty fields that have schema defaults. Preserve nullable
// balances/counts and keep required customer fields subject to validation.
export function normalizeStoredCustomerFields(job:Record<string,unknown>){
 const normalized={...job};
 for(const [key,schema] of Object.entries(fieldsSchema.shape)){
  if(normalized[key]===null&&!schema.safeParse(null).success&&schema.safeParse(undefined).success)delete normalized[key];
 }
 return normalized;
}
