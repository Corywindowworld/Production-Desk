...
test('September reports import once, preserve existing accounts, and reconcile aging',async()=>{
 ...
 const {aging,bonusMetrics}=await vite.ssrLoadModule('/lib/operations.ts');const aged=rows.filter(j=>aging(j,'2026-09-18').aged);assert.equal(aged.length,9);assert.equal(aged.reduce((s,j)=>s+j.amount,0),29719);assert.equal(rows.filter(j=>aging(j,'2026-09-18').missing).length,33);
